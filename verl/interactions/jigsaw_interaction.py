...
def generate_image_with_current_answer(solution_str, ground_truth):
    if '<Final Answer>' in solution_str:
        ...
        try:
            ...
            print(f'Error in final answer: {e}, org solution: {solution_str}')
            return None



class JigSawInteraction(BaseInteraction):
    ...
    async def generate_response(
        ...
        reward, format_score, shape_score, final_answer_score = await self.calculate_score(instance_id)
        # ensemble_image = generate_image_with_current_answer(content, self._instance_dict[instance_id]["ground_truth"])
        if reward == 1.0:
            ...
            if format_score != 1.0:
                response += "Please use the format provided to you to think and generate your final answer. "
            # if shape_score != 1.0:
            ...
            if final_answer_score != 1.0:
                response += f"Finally think more about your final answer, your current answer correctly identifies {final_answer_score * 4} jigsaw pieces."
            should_terminate_sequence = False

        print(f"Response for this iteraction: {response}")
        return should_terminate_sequence, response, reward, {} #, None
