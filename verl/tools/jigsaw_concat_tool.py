...
class JigsawRestorationTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        _tool_schema = OpenAIFunctionToolSchema.model_validate({
            ...
            "function": {
                ...
                "description": "Restores the jigsaw puzzle by identifying which piece from the current board belongs to each position in the solved puzzle.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "order": {
                            ...
                            "items": {
                                ...
                                "minItems": 2, 
                                "maxItems": 2,
                                "description": "The (row, col) coordinate of the piece on the CURRENT board. The coordinates is 1-based."
                            },
                            "minItems": 4,
                            "maxItems": 4,
                            "description": (
                                "A list of coordinates representing the solution sequence in row-major order. "
                                "The list index corresponds to the TARGET slot in the restored puzzle (0: top-left, 1: top-right, 2: bottom-left, 3: bottom-right). "
                                "The value at each index is the (row, col) coordinate of the piece on the CURRENT board that belongs in that slot. "
                                "The coordinates is 1-based. "
                                "Example: [(1,2), (2,1), ...] means the piece currently at (1,2) belongs in the top-left slot, and the piece at (2,1) belongs in the top-right slot."
                            ),
                        }
                    },
                    "required": ["order"],
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        try:
            ...
            print(instance_id, 'reconstructed answer: ', final_answer)
